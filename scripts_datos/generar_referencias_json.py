...
def extraer_referencias():
    ...
    # 1. Extraer jerarquías de relaciones
    cursor.execute('''
    SELECT DISTINCT
        nombre_padre,
    ...
    # 2. Identificar totales y subtotales en FPI
    cursor.execute('''
    SELECT
        id,
        descripcion,
    ...
    WHERE es_subtotal = 1 OR es_total = 1
    ORDER BY nivel_jerarquia, descripcion
    ''')
    ...
    # 3. Totales de formación por nivel
    cursor.execute('''
    SELECT
        id,
        nivel_formacion,
    ...
    for row in cursor.fetchall():
        nivel_totales.append({
            ...
            "componentes": {
                "regular": {"meta": row['regular_meta'], "ejecucion": row['regular_ejecucion']},
                "campesena": {"meta": row['campesena_meta'], "ejecucion": row['campesena_ejecucion']},
                "full_popular": {"meta": row['full_popular_meta'], "ejecucion": row['full_popular_ejecucion']}
            },
    ...
    # 4. Métricas adicionales que son totales
    cursor.execute('''
    SELECT DISTINCT
        categoria,
    ...
    for row in cursor.fetchall():
        ...
        # Obtener detalles de métricas en esta categoría
        cursor.execute('''
        SELECT
            id,
            nombre_metrica,
    ...
    # 5. Calculos especiales conocidos
    referencias['calculos']['formulas_conocidas'] = {
        ...
        "TOTAL FORMACIÓN LABORAL (D=A+B+C+T)": {
            ...
            "componentes": ["SubTotal Auxiliares (A)", "SubTotal Operarios (B)", "SubTotal Técnico Laboral (C)", "Profundización Técnica (T)"]
        },
        "TOTAL FORMACION TITULADA (F = D+E)": {
            "formula": "TOTAL FORMACIÓN LABORAL (D=A+B+C+T) + EDUCACION SUPERIOR (=E)",
            "componentes": ["TOTAL FORMACIÓN LABORAL (D=A+B+C+T)", "EDUCACION SUPERIOR (=E)"]
        },
        "SubTotal Programa de Bilinguísmo (K = I + J)": {
            "formula": "Programa de Bilingüismo - Virtual (I) + Programa de Bilingüismo - Presencial (J)",
            "componentes": ["Programa de Bilingüismo - Virtual (I)", "Programa de Bilingüismo - Presencial (J)"]
        },
        "TOTAL FORMACION COMPLEMENTARIA (N = G+H+K+L+M)": {
            "formula": "Formación Complementaria - Virtual (G) + Formación Complementaria - Presencial (H) + SubTotal Programa de Bilinguísmo (K) + Formación Complementaria CampeSENA (L) + Formación Complementaria Full Popular (M)",
            "componentes": [
        ...
        "TOTAL FORMACION PROFESIONAL INTEGRAL (O=N+F)": {
            ...
            "componentes": ["TOTAL FORMACION TITULADA (F = D+E)", "TOTAL FORMACION COMPLEMENTARIA (N = G+H+K+L+M)"]
        }
    }

    # 6. Mapeo de programas especiales
    cursor.execute('''
    ...
    # 7. Indicadores transversales (aparecen en múltiples tablas)
...
def main():
    ...
    print(f"  - Totales FPI: {len(referencias['totales']['formacion_profesional_integral'])}")
    print(f"  - Totales por nivel: {len(referencias['totales']['formacion_por_nivel_programa'])}")
    print(f"  - Categorias de metricas: {len(referencias['totales']['metricas_adicionales'])}")
    print(f"  - Programas especiales: {len(referencias['programas_especiales'])}")
    print(f"  - Formulas conocidas: {len(referencias['calculos']['formulas_conocidas'])}")
